export function classifyStage({ text = "", previousStage = null } = {}) {
  ...
  if (/calote|golpe|zoeira|brincadeira|kkk|kkkk|s[oó]\s+pesquisando|s[oó]\s+olhando|s[oó]\s+queria\s+saber|por\s+curiosidade|sem\s+compromisso|talvez\s+(?:mais\s+pra\s+frente|futuramente)/iu.test(normalizedText)) return "curioso";
